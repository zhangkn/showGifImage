...
@end
@implementation KNPictureBrowseView
...
- (void)setupImageView:(NSMutableArray *)array{
    ...
    for (KNPhotoZoom *photoZoom in self.scrollView.subviews) {
    ...
    for (int i = 0; i < array.count ; i++) {
        ...
        KNPhotoZoom * photoZoomView = [[KNPhotoZoom alloc] initWithFrame:CGRectMake(i * self.frame.size.width, 0, self.frame.size.width, self.frame.size.height)];
    ...
    self.currentPhotoZoom = (KNPhotoZoom *)(self.scrollView.subviews.firstObject);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if (scrollIndex != self.index) {
        //重置上一个缩放过的视图
        KNPhotoZoom * zoomView  = (KNPhotoZoom *)scrollView.subviews[self.index];
        [zoomView pictureZoomWithScale:1.0];
        
        self.index = scrollIndex;
        self.currentPhotoZoom = (KNPhotoZoom *)scrollView.subviews[self.index];
